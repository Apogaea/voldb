describe("GridCells.collumnDenominators", function() {
    ...
    it("A row whose columns are all 60 should have denominators 1, 60, 2, 30, 3, 20, 4, 15, 5, 12, 6, 10", function() {
        ...
        chai.expect(denominators).to.eql(expected);
        chai.expect(_.intersection(expected, denominators)).to.eql(expected);
        chai.expect(_.isEmpty(_.difference(expected, denominators))).to.be.true;
    });
