def test_cancelled_shifts_are_not_included(factories, user):
    ...
    assert len(user.get_shifts()) == 2
    assert shift_a in user.get_shifts()
    assert shift_b not in user.get_shifts()
    assert shift_c in user.get_shifts()
...
def test_filtered_to_only_current_event_shifts(settings, factories, user, models):
    ...
    assert shift_b not in user.get_shifts()
    assert shift_c in user.get_shifts()
    assert past_shift not in user.get_shifts()
