...
def test_event_list_page(admin_webtest_client, models, factories):
    ...
    new_open_at = event.registration_open_at.replace(microsecond=0) + timezone.timedelta(10)
    new_close_at = event.registration_close_at.replace(microsecond=0) + timezone.timedelta(20)

    response.form['name'] = 'New Test Event Name'
    response.form['registration_open_at'] = to_current_timezone(new_open_at).strftime('%Y-%m-%d %H:%M:%S')
    response.form['registration_close_at'] = to_current_timezone(new_close_at).strftime('%Y-%m-%d %H:%M:%S')
